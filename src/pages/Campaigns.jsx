    ...
    <div>
      ...
      <div className="bg-white rounded-2xl border border-[#EBEBF0] shadow-[0_2px_8px_0_rgba(0,0,0,0.012)] p-6 mb-6">
         ...
         <div className="grid grid-cols-2 divide-x divide-[#EBEBF0] mb-4">
           <div className="pr-6">
             ...
             <p className="text-3xl font-bold tracking-tight mb-1" style={{ color: '#27272b' }}>€1,032</p>
           </div>
           <div className="pl-6 text-right">
             <p className="text-xs text-[#5b616e] mb-1">Remaining in fund</p>
             <p className="text-3xl font-bold tracking-tight mb-1" style={{ color: '#27272b' }}>€368</p>
           </div>
         </div>

         {/* Progress bar */}
         <div className="w-full h-1.5 rounded-full overflow-hidden flex mb-2" style={{ background: '#D1D5DB' }}>
           <div className="h-full rounded-full" style={{ width: '73.7%', background: '#534AB7' }} />
         </div>
         ...
         <div className="flex items-center justify-between mb-4">
           <p className="text-xs flex items-center gap-1.5" style={{ color: '#27272b' }}>
             ...
             73.7% paid out of €1,400 Campaign Fund
           </p>
      ...
      <div>
        ...
        <div className="bg-white rounded-2xl border border-[#EBEBF0] shadow-[0_2px_8px_0_rgba(0,0,0,0.012)] overflow-hidden">
          {/* Table Header */}
          <div className="grid grid-cols-[2fr_1fr_1fr_1.2fr_120px_48px] px-6 py-3 bg-[#F7F7F7] border-b border-[#EBEBF0]">
          ...
          {filtered.map((campaign, idx) => {
            ...
            const barColor = campaign.status === 'completed' ? '#5F5E5A' : campaign.status.startsWith('paused') ? '#BA7517' : '#534AB7';
            const rowOpacity = campaign.status === 'completed' ? 0.6 : 1;
            
            const statusStyles = {
              active: { bg: '#EAF3DE', text: '#3B6D11' },
              paused_manual: { bg: '#FAEEDA', text: '#854F0B' },
              paused_budget: { bg: '#FAEEDA', text: '#854F0B' },
              completed: { bg: '#F1EFE8', text: '#5F5E5A' }
            };
            const statusStyle = statusStyles[campaign.status] || statusStyles.active;
            
            return (
              <div key={campaign.id}>
                <div
                  className="grid grid-cols-[2fr_1fr_1fr_1.2fr_120px_48px] px-6 py-4 items-center hover:bg-[#F5F3FC] transition-colors cursor-pointer"
                  ...
                  <div>
                    <div className="w-full h-1.5 rounded-full overflow-hidden mb-1" style={{ background: '#D1D5DB' }}>
                      <div className="h-full rounded-full" style={{ width: `${Math.min(unitsPct, 100)}%`, background: barColor }} />
                    </div>
                  ...
                  <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                    <span className="inline-block px-3 py-1 rounded-full text-xs font-semibold" style={{ backgroundColor: statusStyle.bg, color: statusStyle.text }}>
                      {campaign.status === 'paused_manual' ? 'Paused' : campaign.status === 'paused_budget' ? 'Paused' : campaign.status.charAt(0).toUpperCase() + campaign.status.slice(1)}
                    </span>
