...
export default function MetricStatCard({ label, values, icon: Icon, trend, trendUp, className, to }) {
  ...
  <div>
      ...
      <span className={cn("text-xs font-medium", trendUp ? "text-emerald-600" : "text-red-500")}>
        ...
        <div className="relative ml-auto">
          ...
          {open &&
        <div className="absolute right-0 bottom-full mb-1 bg-white border border-[#EBEBF0] rounded-lg shadow-md z-10 min-w-[110px]">
              {PERIODS.map((p) =>
          <button
            key={p}
  ...
  const element =
  <div className="bg-white p-5 opacity-100 rounded-2xl shadow-[0_2px_8px_0_rgba(0,0,0,0.012)] cursor-pointer hover:shadow-md transition-shadow">
      {content}
    </div>;
